...
def main():
    ...
    def get_game_frame(window, mqtt_sender):
        ...
        enter_button["command"] = lambda: handle_word_entry(mqtt_sender, word_display['text'], word_entry, word_display)
...
###############################################################################
def handle_word_entry(mqtt_sender, word, word_entry, word_display):
    print("Input entered, checking and responding to input.")
    if len(word) != len(str(word_entry.get())):
        print("Word incorrect, stopping")
        mqtt_sender.send_message("m2_stop")
        return
    else:
        for k in range (len(str(word_entry.get()))):
            if str(word_entry.get())[k] != word[k]:
                print("Word incorrect, stopping")
                mqtt_sender.send_message("m2_stop")
                return
